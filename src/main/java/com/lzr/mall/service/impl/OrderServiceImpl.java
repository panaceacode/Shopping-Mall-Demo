...
public class OrderServiceImpl implements IOrderService {
    ...
    public ResponseVo<OrderVo> create(Integer uid, Integer shippingId) {
        ...
        // 获取购物车，校验（商品库存等）
        List<Cart> cartList = cartService.cartList(uid).stream()
        ...
        // 计算价格，只计算被选中的商品
        // 生成订单，入库：order和order_item,需要用事务，保证两个写入的状态一致
        ...
        // 构造orderVo对象
        OrderVo orderVo = buildOderVo(order, orderItemList, shipping);
    ...
    public ResponseVo<PageInfo> list(Integer uid, Integer pageNum, Integer pageSize) {
        ...
        for (Order order : orderList) {
            OrderVo orderVo = buildOderVo(order,
    ...
    private OrderVo buildOderVo(Order order, List<OrderItem> orderItemList, Shipping shipping) {
